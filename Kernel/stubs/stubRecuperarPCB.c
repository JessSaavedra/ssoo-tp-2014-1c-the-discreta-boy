...
void stubRecuperarPCB(void){
	...
	getaddrinfo(IPLOCAL,"7008", &criteriosConfiguracion,&datosSocketServidor);
	...
	log_debug(logDebug,"stubRecuperarProceso a punto de enviar");

	send(socketPCP,streamPCB,10*TAMANIO_INT,0);

	log_debug(logDebug,"stubRecuperarProceso finalizo");
